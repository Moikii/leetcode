...
## Problem Summary
The task is to design a data structure that can efficiently manage data packets in a network router. Different methods have to be implemented to handle incoming packages and forward packages. Additionally a method that counts packets with a certain destination still in the queue has to be implemented.
...
In the end, I ended up with a `deque` to keep track of all the packets, a `set` to check duplicate packages efficiently and a `dict` where each key is a destination that points to a list of timestamps and a *start-index* that changes the start of the list, rather than removing elements from the list itself.